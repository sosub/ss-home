...
class VideoListExtra extends Component {
          ...
          <thead>
            <tr>
              ...
              <TH>
                <Hide xs>
                  <IconButton
                    onClick={() => this.changeOrderBy('view_amount')}
                    disabled={!changeOrderBy}
                  >
                    <span>LƯỢT XEM</span>
                    {orderBy.indexOf('view_amount') > -1 ? (
                      <span>{orderBy[0] === '-' ? '↓' : '↑'}</span>
                    ) : (
                      <span />
                    )}
